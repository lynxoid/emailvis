...
pv.Force.spring = function(k) {
  ...
  force.links = function(x) {
    if (arguments.length) {
      ...
      kl = x.map(function(l) {
			if (l.value)
				return 0.1 - Math.pow( 0.3 / l.value, 2);
			else
...
pv.Layout.Network.prototype.buildImplied = function(s) {
  ...
  if (s.links)
	  s.links.forEach(function(d) {
		  var v = d.linkValue;
		  var node = (d.sourceNode || (d.sourceNode = s.nodes[d.source]));
		  if (node)
			node.linkDegree += v;
			
		  var node = (d.targetNode || (d.targetNode = s.nodes[d.target]));
		  if (node)
			node.linkDegree += v;
...
pv.Behavior.drag = function() {
  ...
  //pv.listen(window, "mouseout", mouseup);
  return mousedown;
};
/**
